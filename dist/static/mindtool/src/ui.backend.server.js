MM.UI.Backend.Server = Object.create(MM.UI.Backend, {
	id: {value: "server"}
});

MM.UI.Backend.Server.init = function(select) {
...
MM.UI.Backend.Server.handleEvent = function(e) {
...
MM.UI.Backend.Server.show = function(mode) {
...
MM.UI.Backend.Server.setState = function(data) {
	this._load(data.id);
}

MM.UI.Backend.Server.getState = function() {
	var data = {
		b: this.id,
		id: MM.App.map.getId()
	};
...
MM.UI.Backend.Server.save = function() {
...
MM.UI.Backend.Server.load = function() {
	this._load(this._list.value);
}

MM.UI.Backend.Server._load = function(id) {
	try {
